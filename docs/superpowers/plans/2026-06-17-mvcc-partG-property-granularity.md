...
> **SCOPE CORRECTION (2026-06-17, discovered in execution):** the knob is **symmetric** — property tags are needed on **both reads AND writes**, not reads alone. The rw-antidependency check compares the *read's* `PropTag` against the *write's* `PropTag`; if writes stayed entity-level (`None` = wildcard), `prop_compatible(read_prop, None)` is always `true` and the disjoint-property scan case still false-aborts. So `write_set` ALSO carries `PropTag` and the `WriteTracker` + single-property mutation operators (`SET n.balance`) record the property — **but the W-W lost-update check still compares ENTITIES ONLY (ignores the tag)**. This expands Task 2 (both `read_set` + `write_set`) and Task 3 (both `ReadTracker` + `WriteTracker`). Whole-entity/multi-property/structural mutations (delete, label set, `SET n = {...}`) record `None` (entity-level). Under entity-level policy everything is `None` → unchanged.
