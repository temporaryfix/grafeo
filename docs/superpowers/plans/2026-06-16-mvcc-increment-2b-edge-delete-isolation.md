...
> **[RESOLVED during execution — NO CODE CHANGE NEEDED.]** The plan mis-identified the path. Transactional `DETACH DELETE` does NOT go through `LpgStore::delete_node_edges`. Real path: `DeleteNodeOperator::next` (`execution/operators/mutation.rs:~815`, `detach==true`) iterates incident edges via `edges_from` and deletes each with `delete_edge_versioned(eid, epoch, tx)` where `tx = self.transaction_id.unwrap_or(SYSTEM)`. The dispatch `delete_edge_versioned` (`graph_store_impl.rs:523`) routes `tx == SYSTEM` → eager `delete_edge_at_epoch`, else → deferred `delete_edge_transactional`. So once Task 1 made `delete_edge_transactional` defer, transactional DETACH automatically routes through the deferred path; auto-commit DETACH (`transaction_id == None` → `tx == SYSTEM`) stays eager. `delete_node_edges` is OFF this path (store unit tests + layered/WAL/CDC wrappers only) and MUST NOT be touched (eager-semantics callers). **Gate verified GREEN at HEAD `9b7cbb78` with no change:** `uncommitted_detach_delete_edges_invisible_to_other_sessions` passes; full `mvcc_isolation` = 10 pass / 1 fail (`committed_edge_delete`, needs Task 3). The two `~~Files~~`/Step bullets below are SUPERSEDED.

**Files:** ~~`node_ops.rs` (`delete_node_edges`)~~ (superseded — see note above)
