...
- **as-of-E scoring requires `temporal`:** `read_node_property_visible` returns the snapshot value only with the `temporal` feature (the property version history). Without `temporal`, vector reads — like *all* property reads in the engine — return committed-latest; the as-of-E (re-embed) guarantee is a `temporal`-mode property, consistent with the rest of the engine, and the `EntityId::Index` anti-phantom recording still fires either way. The re-embed acceptance test is `#[cfg(feature = "temporal")]` accordingly.
