...
- **A failed `commit()` (write-write conflict) leaked the transaction and stalled MVCC GC.** The conflict path replayed the property undo log but never discarded the transaction's uncommitted (PENDING) node/edge versions and never marked the transaction aborted. The zombie `Active` transaction pinned `min_active_epoch`, so version GC stopped reclaiming entirely, and its stale write-set was re-scanned on every subsequent write. The conflict path now discards uncommitted versions and aborts the transaction. (Audit Wave 1)
- **NULL join keys spuriously matched in outer joins.** `HashJoinOperator` inserted NULL build keys for `Left`/`Right`/`Full` joins, so a NULL probe key matched them and `NULL = NULL` joined rows, violating three-valued logic (filters already handled this correctly). NULL keys are no longer inserted for any join type; outer-join unmatched-row emission is unaffected (it runs off match-tracking, not the hash table). (Audit Wave 1)
- **Join reorder dropped a filtered relation's WHERE predicate.** When flattening a `Join` tree, `collect_join_tree` recursed into a `Filter` and discarded its predicate, so reordering a join whose relation carried a filter silently lost the constraint. A filter wrapping a single base relation now travels with that relation through reordering; a filter spanning a join declines the flatten so reordering is skipped and the original plan is preserved. (Audit Wave 1)
- **`Serializable` isolation silently behaved as Snapshot Isolation.** `record_read` was never called from the live execution path, so the commit-time SSI validation never fired. `begin` (direct API and GQL) now returns a clear "not yet supported — use SnapshotIsolation" error rather than shipping a guarantee the name implies but does not deliver. Real SSI is tracked for the Wave 2 isolation rework. (Audit Wave 1)
- **`CREATE GRAPH x AS COPY OF y` silently produced an empty graph.** `LpgStore::copy_graph` was a no-op that returned `Ok`. It now performs a real deep copy of nodes (labels + properties), edges (type + properties, with remapped endpoints), and property indexes; copying a graph onto itself is a no-op. Vector/text indexes are not carried by the copy (recreate them on the destination if needed). (Audit Wave 1)
- **Layered store: a promoted entity deleted after `compact()` resurfaced in traversals.** `delete_edge`/`delete_node` (and the `_versioned` variants) on an entity that had been promoted into the overlay (e.g. by a property write) deleted only the overlay copy and never tombstoned the base copy, so the deleted edge/node reappeared via `get_edge`/`get_node`, `edges_from`, and `neighbors`, and a second delete returned `false`. Deletes now remove the overlay copy *and* tombstone the base copy when present. Relatedly, `neighbors()` hand-rolled a tier merge that filtered node tombstones but never per-edge tombstones; it now delegates to `edges_from()` so a target reachable only via a deleted base edge is no longer reported.
- **`extract_subgraph` and `remove_orphan_edges` were blind to the base tier after `compact()`.** Both read the overlay-only `LpgStore`, so a base-tier node errored as "does not exist in source database", a promoted node's pre-compact base edges were silently dropped from the extract, and `remove_orphan_edges` could delete a valid edge whose destination is a base-tier node. Both now read through a tier-merged `read_graph_view`. Per-property history (under `temporal`) still comes from the mutable store, falling back to current values for the flattened columnar base.
- **Post-compact vector/text/property search could leak a base-node deletion as a stale hit.** `graph_store_ref` returned the overlay store, bypassing `LayeredStore`'s `deleted_from_base` tombstone filter; it now uses the merged view.
- **`ORDER BY` + `LIMIT` fused a sort on the wrong column when a `RETURN` alias collided with a synthetic property-column name.** `RETURN n.foo AS n_age ORDER BY n.age LIMIT k` sorted by `n.foo`, because the heap TopK rewrite matched sort keys to projected columns by formatted name (`n_age`). Sort keys are now matched against the projected expressions structurally, so an alias can no longer satisfy a sort key it does not actually project; unmatched keys fall through to the (correct) unfused sort.
- **`open_multi` silently dropped a conflicting vector-index configuration.** When two snapshots declared the same `(label, property)` index with different dimensions/metric/build params, the first won and the merged index then failed to rebuild entirely (warn-only), leaving `open_multi` returning `Ok` with no index. Conflicts are now rejected at merge time (consistent with `merge_snapshot_schemas`), and index-rebuild failures propagate on the `open_multi` path while single-snapshot import stays lenient. The `open_multi` rustdoc was also corrected (stale "not yet enforced" notes removed; the bogus `# Panics if empty` replaced with the real `Err`).
- **`write_zigzag_gamma(i64::MIN)`** wrapped to `write_gamma(0)` in release builds, corrupting the WebGraph bit stream. It now folds via the canonical `codec::delta::zigzag_encode`/`zigzag_decode` (identical wire format for every valid input) and panics with a clear message at the single unrepresentable boundary (a first-gap of `i64::MIN`, which requires a graph of > 2^63 nodes).
- **WASM codec node ids were truncated `u64` → `u32`.** `RabitqCodec.search` and the `WebGraphCodec` successor/count surface cast node ids to `u32`, silently corrupting ids ≥ 2^32 from natively-built blobs. They now cross to JS as `f64` (exact for any id below 2^53).

### Performance

- **Conjunctive filters now split before pushdown.** `Filter(A AND B)` was pushed as a single unit, stranding a conjunct that could anchor on one relation above a cartesian product (e.g. `MATCH (a),(b) WHERE a.x = 1 AND b.y = 2`). The optimizer now splits the top-level `AND` into conjuncts and pushes each independently, so each predicate anchors on its own scan (and each single-predicate filter then receives its own index/range pushdown hint). Results are unchanged. (Audit Wave 1)
- **FSST encode** no longer scans all 255 symbols at every input position: a per-first-byte bucket index (built once at table construction, longest-first) replaces the linear scan, with the exhaustive scan kept as a verified-equivalent fallback. `SymbolTable::train` now counts substrings by borrowing the sample instead of allocating a `Vec<u8>` per occurrence.
- **RaBitQ query** path stops allocating a `Vec<u64>` per stored vector per query (`RabitqView::search` reuses one code buffer across the coarse scan), and both the owned and view coarse passes use `select_nth_unstable` + a partial sort (O(N + n log n)) instead of fully sorting all candidates. View↔owned parity and recall-floor tests confirm identical results.
